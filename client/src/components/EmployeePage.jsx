...
export default function EmployeePage({ role }) {
  // Flag penanda privilege super admin
  ...
  const handleSaveEmployee = async (e) => {
    e.preventDefault();
    if (!isSuperAdmin) return; // Guard proteksi akses
  ...
  const filteredEmployees = employees.filter(
    (emp) =>
      ...
      emp.badgeNumber.includes(searchQuery)
  );

  return (
    <div className={`max-w-7xl mx-auto my-6 px-4 grid grid-cols-1 ${isSuperAdmin ? "lg:grid-cols-3" : "grid-cols-1"} gap-6 font-sans`}>
      
      {/* FORM INPUT HANYA TAMPIL JIKA LOGIN SEBAGAI SUPER ADMIN */}
      {isSuperAdmin && (
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm h-fit">
          <h2 className="text-lg font-bold text-gray-800 mb-1">
            {editingId ? "🛠️ Edit Data Karyawan" : "👥 Tambah Karyawan Baru"}
          </h2>
          <p className="text-xs text-gray-400 mb-4">
            Pengelolaan data registrasi personil lapangan langsung ke Database.
          </p>
          ...
          <form onSubmit={handleSaveEmployee} className="space-y-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                required
                className="border border-gray-200 p-3 rounded-xl bg-gray-50/50 focus:bg-white focus:border-red-500 focus:ring-1 focus:ring-red-500 outline-none transition-all text-sm font-bold text-gray-700 cursor-pointer"
              >
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                required
                className="border border-gray-200 p-3 rounded-xl bg-gray-50/50 focus:bg-white focus:border-red-500 focus:ring-1 focus:ring-red-500 outline-none transition-all text-sm font-semibold text-gray-700 cursor-pointer"
              >
                <option value="" disabled>
                  -- Pilih rute tujuan --
                </option>
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button
                ...
                className={`font-bold py-2.5 rounded-xl text-xs text-white shadow-md transition-all ${
                  editingId ? "w-1/2 bg-amber-500 hover:bg-amber-600 shadow-amber-100" : "w-full bg-red-600 hover:bg-red-700 shadow-red-100"
                }`}
              >
